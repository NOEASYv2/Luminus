...
bool __stdcall CreateMove(float SampleTime, CUserCmd* pCmd)
{
	...
	cfg.Rage.AntiAim.untrusted = true;	
	...
	if (!legitBotUsingTick && aimbotV2->antiLeak == true)
	...
	{
		static bool lastTpBind = G::pressedKeys[VK_MBUTTON];

		if (lastTpBind != G::pressedKeys[VK_MBUTTON] && G::pressedKeys[VK_MBUTTON])
			cfg.Misc.thirdPerson = !cfg.Misc.thirdPerson;

		lastTpBind = G::pressedKeys[VK_MBUTTON];
	}
