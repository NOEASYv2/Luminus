...
#include "MenuV2_SkinChangerTab.h"
...
void MenuV2::Draw()
{
	...
	if (cfg.showMenu)
	{
		...
		if (ImGui::Begin(xorstr("LUMINUS"), &cfg.showMenu, ImVec2(800, 600), 0.8f, ImGuiWindowFlags_NoCollapse))
		{
			...
			tabNames.push_back(MENU_TRANSLATE("Skins", "Skins"));
			tabNames.push_back(MENU_TRANSLATE("PList", "PList"));
			tabNames.push_back(MENU_TRANSLATE("Config", "Config"));			
			...
			ImGui::TabLabels(tabNames, selectedTab, true);
			{
				...
				ImGui::BeginChild(xorstr("All"), ImGui::GetContentRegionAvail(), true);
				{
					...
					case 4: DrawSkinChangerTab(); break;
					case 5: DrawPlayerListTab(); break;
...
void MenuV2::ResetStyle()
{
	...
	style.Colors[ImGuiCol_ButtonHovered] = ImVec4(RGBA_TO_FLOAT(95, 0, 45, 255));
	style.Colors[ImGuiCol_ButtonActive] = ImVec4(RGBA_TO_FLOAT(150, 0, 50, 255));
	...
	style.Colors[ImGuiCol_CloseButton] = ImVec4(0.55f, 0.10f, 0.10f, 1.00f);
	style.Colors[ImGuiCol_CloseButtonHovered] = ImVec4(0.55f, 0.20f, 0.20f, 0.59f);
	//style.Colors[ImGuiCol_ChildWindowBg] = ImVec4(RGBA_TO_FLOAT(20, 20, 20, 255));
