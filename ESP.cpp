...
void ESP::DrawWeapon(CBaseEntity *entity)
{
	...
	if (GetPlayerBox(entity, x, y, width, height))
	{
		...
		color = entity->IsDormant() ? D3D_COLOR_WHITE(GetDormantAlpha(entity)) : D3D_COLOR_WHITE(cfg.Visual.espAlpha + 55);
		//render->DrawString(_x, _y, color, "%d", (int)choked);

		float max = (y + height) - _y;
		float ch = (max * (15 - choked)) / 15;

		render->DrawFilledRectangle(_x, _y, (y + height) - _y, 4, D3D_COLOR_BLACK(entity->IsDormant() ? GetDormantAlpha(entity) : cfg.Visual.espAlpha + 55));
		render->DrawFilledRectangle(_x + 1, _y + ch + 1, (y + height) - _y - ch - 2, 4 - 2, color);
		...
		color = entity->IsDormant() ? D3D_COLOR_WHITE(GetDormantAlpha(entity)) : D3D_COLOR_WHITE(cfg.Visual.espAlpha + 55);
		render->DrawString(_x, _y, color, "%s", state.c_str());

		_y += 8;

		color = entity->IsDormant() ? D3D_COLOR_WHITE(GetDormantAlpha(entity)) : D3D_COLOR_ORANGE(cfg.Visual.espAlpha + 55);
		if (entity->IsScoped())
		render->DrawString(_x, _y, color, "%s", "SCOPE");
		}
		}
		}*/
	}
