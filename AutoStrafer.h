...
public:
	...
	void MovementFix(CUserCmd* cmd, QAngle& realvec) 
	{
		...
		if (90 < abs(cmd->viewangles.x)) cmd->forwardmove *= -1;
	}

	static void BhopMovement(CUserCmd* pCmd, QAngle& OrigAngles) 
	{
		static AutoStrafer strafer;

		CBaseEntity* pLocal = G::pLocal;
		static QAngle LastOrigAng = OrigAngles.Normalize();

		auto airaccel = I::pCVar->FindVar(xorstr("sv_airaccelerate"))->GetFloat();
		auto maxspeed = I::pCVar->FindVar(xorstr("sv_maxspeed"))->GetFloat();

		double yawrad = Math::NormalizeYawInPlace(OrigAngles.y) * M_PI / 180.0f;
		...
		double tau = /*I::pGlobals->absoluteframetime*/I::pGlobals->interval_per_tick, MA = (maxspeed * airaccel) / tau;
		...
		OrigAngles.y = Math::NormalizeYawInPlace((float)yawrad * 180.0f / M_PI);

		pCmd->sidemove = Sdir * 450.0f;
		pCmd->forwardmove = Fdir * 450.0f;

		strafer.MovementFix(pCmd, OrigAngles);
		Math::NormalizeYaw(LastOrigAng.y);
		LastOrigAng = OrigAngles.Clamp();
	}
};
