...
public:
	...
	{
		static PVOID KeyValuesSystemAddr = NULL;

		if (!KeyValuesSystemAddr)
		{
			typedef PVOID(__cdecl* oKeyValuesSystem)();
			oKeyValuesSystem KeyValuesSystem = (oKeyValuesSystem)GetProcAddress(GetModuleHandle(xorstr("vstdlib.dll")), xorstr("KeyValuesSystem"));

			KeyValuesSystemAddr = KeyValuesSystem();
		}

		typedef PVOID(__thiscall* oAllocKeyValuesMemory)(PVOID, size_t);
		return Mem::CallVirtualFunction<oAllocKeyValuesMemory>(KeyValuesSystemAddr, 1)(KeyValuesSystemAddr, iAllocSize);
	}
	...
	void operator delete(PVOID pMem)
	{
		...
		Mem::CallVirtualFunction<oFreeKeyValuesMemory>(KeyValuesSystemAddr, 2)(KeyValuesSystemAddr, pMem);
	}

	void SetString(const char* value)
	{
		static auto key_values_set_string = reinterpret_cast<void(__thiscall*)(void*, const char*)>(Mem::FindPattern(xorstr("client.dll"), xorstr("55 8B EC A1 ? ? ? ? 53 56 57 8B F9 8B 08 8B 01")));
		key_values_set_string(this, value);
	}

	void InitKeyValues(const char* name)
	{
		/*static auto key_values = reinterpret_cast<void(__thiscall*)(void*, const char*)>(Mem::FindPattern(xorstr("client.dll"), xorstr("", "xxxxxxxx?????xxxxx????xxxxxxxxxxxxxxxxxxxxxxxxxxx????xxxxxxxxxxxxxxxxxxxxxxx????xxxxx"));
		key_values(key, name);*/
		...
		InitKeyValuesEx(this, name);
	}

	void SetUint64(const char* keyName, int value, int value2)
	{
		static auto key_values_set_uint64 = reinterpret_cast<void(__thiscall*)(void*, const char*, int, int)>(Mem::FindPattern(xorstr("client.dll"), xorstr("55 8B EC 56 6A 01 FF 75 08")));
		key_values_set_uint64(this, keyName, value, value2);
	}

	const char* GetString(const char *keyName, const char *defaultValue)
	{
		static auto key_values_get_string = reinterpret_cast<const char*(__thiscall*)(void*, const char*, const char*)>(Mem::FindPattern(xorstr("client.dll"), xorstr("55 8B EC 83 E4 C0 81 EC ? ? ? ? 53 8B 5D 08")));
		return key_values_get_string(this, keyName, defaultValue);
	}

	int GetInt(const char *keyName, int defaultValue)
	{
		static auto key_values_get_int = reinterpret_cast<int(__thiscall*)(void*, const char*, int)>(Mem::FindPattern(xorstr("client.dll"), xorstr("55 8B EC 6A 00 FF 75 08 E8 ? ? ? ? 85 C0 74 45")));
		return key_values_get_int(this, keyName, defaultValue);
	}

	KeyValues* FindKey(const char *keyName, bool bCreate = false)
	{
		static auto key_values_find_key = reinterpret_cast<KeyValues*(__thiscall*)(void*, const char*, bool)>(Mem::FindPattern(xorstr("client.dll"), xorstr("55 8B EC 83 EC 1C 53 8B D9 85 DB")));
		return key_values_find_key(this, keyName, bCreate);
	}

	void SetInt(const char *keyName, int Value)
	{
		auto key_int = FindKey(keyName);
		if (key_int)
		{
